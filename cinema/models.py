...
class Ticket(models.Model):
    ...
    def validate_row_and_seat(self, num_row: int,
                              num_seat: int,
                              error_to_raise):
        if not (1 <= num_row <= self.movie_session.cinema_hall.rows):
            raise error_to_raise(
                {
                    "row": "row must be in the range [1, "
    ...
    def clean(self):
        self.validate_row_and_seat(
            self.row, self.seat, ValidationError
        )
